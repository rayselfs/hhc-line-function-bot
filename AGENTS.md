...
- ClamAV is packaged only in the finite scan/refresh image. Production deployment must keep the bot free of retired model/scanner endpoint families, office-network addresses, scanner ports, and signature mounts. Copy only the exact ACA secrets and storage/queue references required by each job.
