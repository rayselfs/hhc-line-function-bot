...
- Legacy automatic resource aliases are retired and must never execute before a function handler. A new explicit lookup always searches; only a validator-authorized active-task refinement may replay a prior opaque reference. Resource memory is deduplicated candidate metadata, not an authoritative response cache.
