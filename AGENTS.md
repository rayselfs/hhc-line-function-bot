...
- SearXNG is only a sheet-music not-found fallback after requester consent. It must not become a general web browsing function and must not save results automatically. An authorized requester with effective `save_resource` permission may explicitly select and confirm a direct HTTPS PDF/JPEG/PNG result. The bot may queue only the confirmed opaque work ID; the finite scan worker owns the safe download, must reject HTML, private/reserved addresses, unsafe redirects, and authenticated downloads, then scan and publish through the sole shared binary publisher.
...
- ClamAV is packaged only in the finite scan/refresh image. Production deployment must keep the bot free of retired model/scanner endpoint families, office-network addresses, scanner ports, and signature mounts. The scan worker must use its minimal loader and secret set; do not give it channel secrets, admin IDs, LLM/Notion credentials, or observability keys. Provision the ClamAV storage key and queue connection directly from their Azure resources rather than copying them from the bot.
