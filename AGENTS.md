...
- ClamAV is packaged only in the finite scan/refresh image. Production deployment must keep the bot free of `OLLAMA_*`, `CLAMAV_HOST`, `CLAMAV_PORT`, `VIRUS_SCAN_*`, office IPs, scanner ports, and signature mounts. Copy only the exact ACA secrets and storage/queue references required by each job.
