...
- Dynamic knowledge routing metadata is bounded and may come only from safe administrator input, source/document titles, and headings. Never expose chunks, URLs, answer content, or person values through planner context or knowledge active-task envelopes. Follow-ups search their active source/document/section first and switch only on current explicit metadata evidence.
