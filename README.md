...
Admin `/last-agent-turns` diagnostics include controlled phases for task-frame state, bounded capability names/count, planner provider/disposition/confidence bucket, validator reason, result-envelope status/anchor count/entity types, and task-frame lifecycle outcome. With `REDIS_URL`, the bounded trace list survives replica changes and restarts; local development otherwise uses the in-memory store. These traces never retain raw messages, people, prompts, filenames, URLs, evidence, tokens, source titles/IDs, or temporary sharing links.
