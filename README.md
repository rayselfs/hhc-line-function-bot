...
Schedule lookup combines validated planner arguments with deterministic query refinement and a resolver registry. `query_schedule` remains the only user-facing schedule function: the current media-team resolver and morning-prayer-family resolver expose product labels rather than Notion/PostgreSQL implementation details. Meeting words such as `晨更` and `仙履奇緣` are evidence, not a forced storage choice; role evidence such as `音控` selects the media roster, family/participant evidence selects the family schedule, and a genuine two-domain match asks the requester to choose. The selected `domainKey` is stored in the requester-scoped task frame so follow-ups continue in the same domain. Notion and structured-text adapters normalize source-specific rows into the same schedule-item shape and preserve each distinct role assignment. Recognized date, month, meeting, role, participant, schedule-type, and media-team terms become typed filters and are removed before residual text search. An empty residual therefore adds no full-text condition. Only a genuinely separate query behavior should introduce a new resolver/refinement adapter and structured result; adding another knowledge topic never does. Function-specific parsing must not accumulate in the router.
